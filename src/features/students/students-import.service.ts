...
export class StudentsImportService {
  constructor(
    ...
    @InjectQueue(QUEUE.STUDENTS_IMPORT)
  ...
  async upload(file: Express.Multer.File): Promise<ImportUploadResult> {
    try {
      ...
      if (headers.length === 0 || rows.length === 0) {
        throw new ErrorHandler('El archivo debe tener al menos una fila de encabezados y una de datos', 400);
      }
      ...
      throw new ErrorHandler('Error al procesar el archivo Excel', 500);
    }
  }
  ...
  async startImport(dto: ImportStartDto, userId: string): Promise<ImportStartResult> {
    ...
    if (!data) {
      throw new ErrorHandler('Sesión de importación expirada. Suba el archivo nuevamente.', 400);
    }
